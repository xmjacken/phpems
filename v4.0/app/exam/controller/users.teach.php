...
class action extends app
{
	...
	private function outscore()
	{
		...
		if($basicid)
		{
			...
			/*$rs = $this->favor->getAllExamHistoryByArgs($args,array('ehusername','ehscore','usertruename'));2016.11.09 by jacken*/
			$rs = $this->favor->getAllExamHistoryByArgs($args,array('ehusername','ehscore','usertruename','ehexam','ehstarttime'));
			$r = array();
			foreach($rs as $p)
			{
				/*$r[] = array('ehusername' => iconv("UTF-8","GBK",$p['ehusername']),'usertruename' => iconv("UTF-8","GBK",$p['usertruename']),'ehscore' => $p['ehscore']);2016.11.09 by jacken*/
				$r[] = array('ehusername' => iconv("UTF-8","GBK",$p['ehusername']),'usertruename' => iconv("UTF-8","GBK",$p['usertruename']),'ehscore' => $p['ehscore'],'ehexam' => iconv("UTF-8","GBK",$p['ehexam']),'ehstarttime' => date('Y-m-d H:i',$p['ehstarttime']));
			}
